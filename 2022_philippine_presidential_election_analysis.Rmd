...
city_winners <- pres_df %>% 
  ...
  ungroup() %>% 
  inner_join(select(pres_df, city, city_clean), by = "city_clean") %>%
  inner_join(select(ph_geo, city_clean, geometry), by = "city_clean") %>%
  distinct(city_clean, winner, geometry)
```
...
```{r}
city_winners_with_geo <- ph_geo %>%
  inner_join(select(pres_df, city, city_clean), by = "city_clean") %>%
  inner_join(select(ph_geo, city_clean, geometry), by = "city_clean") %>%
  distinct(city_clean, winner, geometry)
```

We now have the winnners per city and the coordinates of the cities. We can now do a geocomputation map.

```{r}
ggplot(cities_with_geo$geometry) +
  geom_sf(aes(fill=city_winners$winner))
```
